...
echo "    
    ...
    echo            "
                    <td id = 'tdTestnetStatus'>$net_status</td>
                    <td>$net_symbol</td>
                    <td>$net_APIKey</td>";
...
echo "<h4>Current Status of the Testnet bot </h4>";
$bitmex_bot_status = botStatusDAO::getBitmexTestnetStatus();
if($bitmex_bot_status[0][testnet_on] == 1){
    echo "<label class='success' type='label' id='labelStatusChange'  ><h3 ><font color='green'>ON</font></h3></label>";
}else if($bitmex_bot_status[0][testnet_on] == -1){
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#statusChange').click(function()
        {
            ...
            $.ajax({
                type:"GET",
                url: "/../../samriddhee_ajax/bitmex_range_bot_ui_testnet_ajax.php",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                data: {'command':'change_testnet_status'} ,
                success: function(data) {
                    if(data == 500){
                        alert("Some Error Occurred");
                    }else{
                        console.log(data);
                        if(data.current_testnet_status == 1){
                            $('#labelStatusChange').empty();
                            $('#tdTestnetStatus').empty();
                            $('#labelStatusChange').append("<h3 ><font color='green'>ON</font></h3>");
                            $('#tdTestnetStatus').append("<font color='green'>ON</font>");
                        }else if(data.current_testnet_status == -1){
                            $('#labelStatusChange').empty();
                            $('#tdTestnetStatus').empty();
                            $('#labelStatusChange').append("<h3 ><font color='red'>OFF</font></h3>");
                            $('#tdTestnetStatus').append("<font color='red'>OFF</font>");
                        }
                    }
                }
